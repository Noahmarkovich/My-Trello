...
import { addTask, saveLabel, updateBoard } from "../store/board.actions"
...
export function Labels({ board, currTask, setSidebarAction }) {
    ...
    async function handleChange({ target }) {
        ...
        } else setTaskToEdit((prevTask) => ({ ...prevTask, [field]: [id] }))
    ...
    async function onSubmitNewLabel(ev) {
        ...
        try {
            const currBoard = await saveLabel(currLabel, board._id)
    ...
    return <section className="labels">
        <div className="labels-header">
            {isCreateLabel && <button onClick={() => setIsCreateLabel(false)} className="back-btn-clean"><MdArrowBackIosNew className="back" /></button>}
        ...
        {isCreateLabel && <div className="label-create-preview">
            {currLabel && <Label label={currLabel} />}
        </div>}
                ...
                <ul>
                    {board.labels.map(label =>
                        <li key={label.id} className="label">
                            <input onChange={handleChange} name="labelIds" className="label-input" type="checkbox" id={label.id} checked={taskToEdit.labelIds && taskToEdit.labelIds.includes(label.id)} />

                            <Label label={label} handleChange={handleChange} />
                            <button onClick={() => {
                                setIsCreateLabel(true)
                                setCurrLabel(label)
                            }} className="clean-btn"><img className="edit-icon" src={edit} /></button>
                        </li>)}
                </ul>}

            <button onClick={() => setIsCreateLabel(true)} className="sidebar-btn">Create a new label</button>
        </main>
    </section>
}
